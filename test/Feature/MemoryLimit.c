...
// RUN: %klee --pcache-path=%t.klee-out/cache --output-dir=%t.klee-out --max-memory=20 %t.little.bc > %t.little.log
...
// RUN: rm -rf %t.klee-out
// RUN: %klee --pcache-path=%t.klee-out/cache --output-dir=%t.klee-out --max-memory=20 %t.big.bc > %t.big.log 2> %t.big.err
