...
// RUN: %klee --pcache-path=%t.klee-out/cache -output-dir=%t-bfs.klee-out -search=bfs %t.bc >%t-bfs.out
// RUN: %klee --pcache-path=%t.klee-out/cache -output-dir=%t-dfs.klee-out -search=dfs %t.bc >%t-dfs.out
// RUN: %klee --pcache-path=%t.klee-out/cache -output-dir=%t-bfs-dfs.klee-out -search=bfs -search=dfs %t.bc >%t-bfs-dfs.out
// RUN: %klee --pcache-path=%t.klee-out/cache -output-dir=%t-dfs-bfs.klee-out -search=dfs -search=bfs %t.bc >%t-dfs-bfs.out
