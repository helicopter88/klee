...
namespace {
    ...
    cl::opt<bool>
            CexCacheSuperSet("cex-cache-superset",
                             cl::desc(
                                     "try substituting SAT super-set counterexample before asking the SMT solver (default=false)"),
                             cl::init(false));
...
/*#ifdef ENABLE_KLEE_DEBUG
    std::cout << protoCache->DebugString() << std::endl;
#endif*/
    return true;
}
