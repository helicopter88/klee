...
namespace {
    ...
    TEST(ExprTest, BasicSerialization) {
        const ref<Expr> c_expr = ConstantExpr::alloc(256, 32);
    ...
    TEST(ExprTest, ReadSerialization) {
        ...
        // Constant folding rule with index in bound (4)
        ...
        ref<Expr> readOob = ReadExpr::create(ul1, indexOob);
    ...
    TEST(ExprTest, ConcatSerialization) {
        ...
        ref<Expr> concat1 = ConcatExpr::create4(c300, c200, c100, c200);
        ProtoExpr *protoConcat = concat1->serialize();
        EXPECT_EQ(concat1, Expr::deserialize(*protoConcat));
        ref<Expr> concat2 = ConcatExpr::create(concat1, AddExpr::create(c100, c200));
        ProtoExpr *protoConcat2 = concat2->serialize();
